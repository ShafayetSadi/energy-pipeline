...
def panel(x, y, width, height, title, subtitle):
    ...
    d.add(elm.Rect((0, 0), (width, height), fill=PANEL, lw=1).at((x, y)).color(BORDER))
...
# ================= voltage channel =================
d += (vsrc := elm.SourceSin().up().at((1.0, 7.75)).length(1.75))
...
d += (xf := elm.Transformer(t1=5, t2=5, core=True).right().anchor("p1"))
...
# ================= current channel =================
d += (ct := elm.Transformer(t1=2, t2=6, core=True).right().at((3.1, 3.35)))
...
text(
    11.7,
    4.15,
    "R7 is the internal 66.7 Ω burden.\nDo not add a second external burden.",
    size=8,
    color=MUTED,
)
text(
    11.7,
    3.15,
    "R6 + C3 match the voltage-channel\nfilter to preserve phase alignment.",
    size=8,
    color=MUTED,
)
...
d += (
    c1 := elm.Capacitor(polar=True).down().length(0.95).label("C1\n10 µF", loc="right", fontsize=8)
)
...
text(
    4.35,
    -1.8,
    "V_BIAS shifts bipolar AC waveforms\ninto the STM32 ADC's 0-3.3 V range.",
    size=7,
    color=MUTED,
)
...
d.add(elm.Rect((0, 0), (2.15, 0.95), fill="white", lw=1).at((13.62, -0.82)).color(SECTION))

 ...
 * never been set (reset date 2000-01-01), e.g. real hardware before SNTP.
 * Under Renode with `machine RealTimeClockMode HostTimeUTC` this yields the
 * host's current UTC time. */
...
/* Patched in flash by the emulation launcher (renode/run.sh) to the host's
 * current unix time — the harness stands in for SNTP. Stays 0 on hardware. */
volatile const uint32_t g_epoch_seed = 0;

static void iso8601_now(char *buf, size_t len)
{
    if (epoch_base == 0) {
        uint32_t seed = g_epoch_seed;
        telemetry_set_epoch(seed != 0 ? seed : rtc_read_epoch());
    }
                   ...
                   const char *status)
{
    ...
    int n = snprintf(buf, len,
        ...
        "\"firmware_version\":\"0.1.0\",\"rssi_dbm\":-50}",
        device_id, status, ts);
    return n > 0 ? (size_t)n : 0;
}
